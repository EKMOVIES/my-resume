...
async function loadSkills() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(result.message || 'Could not load skills.');
        }
...
function renderList(items) {
    if (items.length === 0) {
        list.innerHTML = '<p style="color:#6b7280;">No skills added yet.</p>';
        return;
    }
        ...
        <div class="item-card">
            ...
            ${item.level ? `
                <div class="proficiency">
                    <p style="font-size:13px;color:#6b7280;">Level: ${item.level}%</p>
